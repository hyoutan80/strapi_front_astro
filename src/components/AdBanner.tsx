...
export function AdBanner({ ad, className }: AdBannerProps) {
    return (
        <div id={ad.placementId} className={`w-full my-8 flex flex-col items-center justify-center ${className}`}>
            <div
                className="ad-container max-w-full overflow-hidden rounded-xl border border-border/40 [&>iframe]:max-w-full [&>img]:max-w-full [&>img]:w-full [&>img]:h-auto [&>img]:object-cover"
